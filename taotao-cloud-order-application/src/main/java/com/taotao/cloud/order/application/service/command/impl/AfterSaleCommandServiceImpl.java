...
public class AfterSaleCommandServiceImpl implements AfterSaleCommandService {
    ...
    //
    //	// 获取订单货物判断是否可申请售后
    //	OrderItem orderItem = orderItemCommandService.getBySn(sn);
    //
    //	// 未申请售后订单货物才能进行申请
    ...
    //	// 获取售后类型
    //	Order order =
    // OperationalJudgment.judgment(orderCommandService.getBySn(orderItem.getOrderSn()));
    //
    //	// 订单未支付，不能申请申请售后
    ...
    //	afterSaleApplyVOBuilder.applyRefundPrice(CurrencyUtils.div(orderItem.getFlowPrice(),
    // orderItem.getNum()));
    ...
    // }
    //
    // @AfterSaleLogPoint(sn = "#afterSaleSn", description = "'审核售后:售后编号['+#afterSaleSn+']，'+
    // #serviceStatus")
    //// @SystemLogPoint(description = "售后-审核售后", customerLog = "'审核售后:售后编号['+#afterSaleSn+']，'+
    //// #serviceStatus")
    // @Override
    // public Boolean review(String afterSaleSn, String serviceStatus, String remark, BigDecimal
    // actualRefundPrice) {
    //	// 根据售后单号获取售后单
    ...
    //// @SystemLogPoint(description = "售后-买家确认解决", customerLog =
    // "'售后-买家确认解决:单号['+#afterSaleSn+']'")
    ...
    //
    //	// 写入商家信息
    //	OrderItem orderItem = orderItemCommandService.getBySn(afterSaleAddCmd.orderItemSn());
    //	Order order =
    // OperationalJudgment.judgment(orderCommandService.getBySn(orderItem.getOrderSn()));
    ...
    //	// 根据商品skuId及订单sn获取子订单
    //	OrderItem orderItem = orderItemCommandService.getOne(new LambdaQueryWrapper<OrderItem>()
    ...
    //		// 判断当前售后的状态---申请中
    //		case APPLY -> orderItem.setReturnGoodsNumber(orderItem.getReturnGoodsNumber() +
    // afterSale.getNum());
    ...
    //
    //	// 获取售后类型
    //	Order order = orderCommandService.getBySn(orderItem.getOrderSn());
    //	AfterSaleTypeEnum afterSaleTypeEnum =
    // AfterSaleTypeEnum.valueOf(afterSaleAddCmd.serviceType());
    ...
    //				order.getOrderStatus(), OrderStatusEnum.DELIVERED.name(),
    ...
    //		// 遍历售后记录获取售后商品数量
    //		implementList.forEach(a -> orderItem.setReturnGoodsNumber(orderItem.getReturnGoodsNumber() +
    // a.getNum()));
    //	}
    //
    //	// 获取已完成售后订单数量
    ...
    //		// 遍历售后记录获取已完成售后商品数量
    //		completeList.forEach(a -> orderItem.setReturnGoodsNumber(orderItem.getReturnGoodsNumber() +
    // a.getNum()));
    //	}
    // }
    //
    ...
    //	}
    //
    //	orderItemCommandService.update(new LambdaUpdateWrapper<OrderItem>()
